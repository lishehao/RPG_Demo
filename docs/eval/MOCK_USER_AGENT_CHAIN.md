...
orchestration trace, and the eval tool records whether each turn and the whole
episode behaved coherently.
...
`trajectory_judge` is artifact-only today. That means the evaluator writes it to
the JSONL trace and optional summary JSON, but the product runtime does not
persist it into `narrative_agent_events`, SQLite session history, or a public API
response.
...
decisions, and per-turn judge results. Persisting it into
`narrative_agent_events` would require either a trusted evaluator write API or
direct repository access from the eval tool, and would blur product runtime
events with evaluator-owned run artifacts.
...
summary path, and final `TrajectoryJudgeResult` in an eval table or manifest
directory. That keeps the privacy boundary clear while making trajectory
summaries discoverable.
...
The mock-user policy and judges themselves do not call an LLM. In live mode,
the existing narrative runtime can call DeepSeek V4 Flash through the configured
gateway when creating or advancing story turns.
...
- Live runs can incur the existing narrative runtime LLM cost through DeepSeek
  V4 Flash; no live LLM judge or per-NPC LLM agent is enabled by default.
