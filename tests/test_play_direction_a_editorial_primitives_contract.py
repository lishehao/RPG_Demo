...
def test_reviewer_evaluation_drawer_is_gated_and_uses_persisted_evidence() -> None:
    ...
    assert 'aria-label="Reviewer evidence check"' in reviewer_fixture
    assert "Local reviewer evidence check" in reviewer_fixture
    assert "Reviewer evidence fixture" not in reviewer_fixture
    assert "Local reviewer evidence fixture" not in reviewer_fixture
...
def test_normal_runtime_impact_summary_keeps_player_facing_resolution_clarity() -> None:
    ...
    assert "const detailLabel = t(\"play.gameplay_forecast_detail_label\")" in summary
    assert "const repeatsDetailLabel =" in summary
...
def test_normal_play_prefers_backend_gameplay_envelope_with_derived_backup() -> None:
    ...
    assert "minHeight: 44" in styles[styles.index("freeSubmitButton") : styles.index("freeSubmitButtonDisabled")]
    assert "actionPalette.primaryBackground" in styles[styles.index("freeSubmitButton") : styles.index("freeSubmitButtonDisabled")]
    commit_text_button = styles[styles.index("commitTextButton: {") : styles.index("advisorInlineAction: {")]
    assert "minHeight: 44" in commit_text_button
    assert "minWidth: 44" in commit_text_button
    diary_attach_preview = styles[
        styles.index("diaryAttachPreview: {"):
        styles.index("diaryAttachPreviewFilled: {")
    ]
    assert "minHeight: 44" in diary_attach_preview
    assert "minWidth: 44" in diary_attach_preview
...
def test_play_selected_action_expands_card_in_place_with_explicit_confirm() -> None:
    ...
    assert '"play.turn_guide_idle_detail": "Read the goal and pressure above, then open a card to review it; if none fits, write your own move."' in strings
    ...
    assert '"play.turn_guide_idle_detail": "先看上方目标和压力，再打开卡片复核；预设不合适时也可以自己写一步。"' in strings
