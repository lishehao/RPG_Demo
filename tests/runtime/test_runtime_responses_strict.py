...
from tests.helpers.responses_bundles import (
    AlwaysGlobalHelpResponsesBundle,
    DeterministicResponsesBundle,
    InvalidMoveResponsesBundle,
    LowConfidenceResponsesBundle,
    NarrationFailureResponsesBundle,
    RouteFailureResponsesBundle,
)
...
def _runtime(bundle) -> RuntimeService:  # noqa: ANN001
...
def test_fail_forward_for_always_fail_forward_move() -> None:
    pack = _load_pack()
    runtime = _runtime(DeterministicResponsesBundle())
...
def test_route_failure_raises_runtime_route_error() -> None:
    pack = _load_pack()
    runtime = _runtime(RouteFailureResponsesBundle())
    scene_id, beat_index, state, beat_progress = runtime.initialize_session_state(pack)
...
def test_low_confidence_raises_runtime_route_error() -> None:
    pack = _load_pack()
    runtime = _runtime(LowConfidenceResponsesBundle())
    scene_id, beat_index, state, beat_progress = runtime.initialize_session_state(pack)
...
def test_narration_failure_raises_runtime_narration_error() -> None:
    pack = _load_pack()
    runtime = _runtime(NarrationFailureResponsesBundle())
    scene_id, beat_index, state, beat_progress = runtime.initialize_session_state(pack)
...
def test_invalid_move_raises_runtime_route_error() -> None:
    pack = _load_pack()
    runtime = _runtime(InvalidMoveResponsesBundle())
    scene_id, beat_index, state, beat_progress = runtime.initialize_session_state(pack)
...
def test_non_help_text_disallows_global_help_route() -> None:
    pack = _load_pack()
    runtime = _runtime(AlwaysGlobalHelpResponsesBundle())
    scene_id, beat_index, state, beat_progress = runtime.initialize_session_state(pack)
