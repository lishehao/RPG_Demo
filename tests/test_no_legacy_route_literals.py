...
def test_no_legacy_route_literals_in_tests_and_scripts() -> None:
    ...
    for path in _iter_scan_files():
        ...
        if rel not in _ALLOW_LEGACY_LITERAL and _RE_LEGACY_STORIES.search(content):
            violations.append(f"{rel}: contains legacy /stories literal")
        if rel not in _ALLOW_LEGACY_LITERAL and _RE_LEGACY_SESSIONS.search(content):
            violations.append(f"{rel}: contains legacy /sessions literal")
        if rel not in _ALLOW_LEGACY_LITERAL and _RE_LEGACY_ADMIN.search(content):
